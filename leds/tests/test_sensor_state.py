import threading

import pytest

from leds.sensor_state import GSR_PAIRS, NODE_GSR_MAPPING, SensorState


class TestSensorStateInitial:
    def test_cap_initial_length(self):
        state = SensorState()
        cap, _, _ = state.snapshot()
        assert len(cap) == 4

    def test_cap_initial_zeros(self):
        state = SensorState()
        cap, _, _ = state.snapshot()
        assert cap == [0, 0, 0, 0]

    def test_gsr_mag_initial_length(self):
        state = SensorState()
        _, gsr_mag, _ = state.snapshot()
        assert len(gsr_mag) == 6

    def test_gsr_mag_initial_zeros(self):
        state = SensorState()
        _, gsr_mag, _ = state.snapshot()
        assert gsr_mag == [0, 0, 0, 0, 0, 0]

    def test_gsr_phase_initial_length(self):
        state = SensorState()
        _, _, gsr_phase = state.snapshot()
        assert len(gsr_phase) == 6

    def test_gsr_phase_initial_zeros(self):
        state = SensorState()
        _, _, gsr_phase = state.snapshot()
        assert gsr_phase == [0, 0, 0, 0, 0, 0]


class TestSetCap:
    @pytest.mark.parametrize("pad_index", [0, 1, 2, 3])
    def test_set_cap_updates_correct_slot(self, pad_index):
        state = SensorState()
        state.set_cap(pad_index, 0.75)
        cap, _, _ = state.snapshot()
        assert cap[pad_index] == pytest.approx(0.75)

    @pytest.mark.parametrize("pad_index", [0, 1, 2, 3])
    def test_set_cap_does_not_affect_other_slots(self, pad_index):
        state = SensorState()
        state.set_cap(pad_index, 0.5)
        cap, _, _ = state.snapshot()
        other = [cap[i] for i in range(4) if i != pad_index]
        assert all(v == 0 for v in other)

    def test_set_cap_overwrites_previous_value(self):
        state = SensorState()
        state.set_cap(2, 0.3)
        state.set_cap(2, 0.9)
        cap, _, _ = state.snapshot()
        assert cap[2] == pytest.approx(0.9)


class TestSetGsr:
    @pytest.mark.parametrize("global_pair_index", range(6))
    def test_set_gsr_updates_mag(self, global_pair_index):
        state = SensorState()
        state.set_gsr(global_pair_index, mag=0.6, phase=1.2)
        _, gsr_mag, _ = state.snapshot()
        assert gsr_mag[global_pair_index] == pytest.approx(0.6)

    @pytest.mark.parametrize("global_pair_index", range(6))
    def test_set_gsr_updates_phase(self, global_pair_index):
        state = SensorState()
        state.set_gsr(global_pair_index, mag=0.6, phase=1.2)
        _, _, gsr_phase = state.snapshot()
        assert gsr_phase[global_pair_index] == pytest.approx(1.2)

    @pytest.mark.parametrize("global_pair_index", range(6))
    def test_set_gsr_does_not_affect_other_mag_slots(self, global_pair_index):
        state = SensorState()
        state.set_gsr(global_pair_index, mag=0.5, phase=0.0)
        _, gsr_mag, _ = state.snapshot()
        other = [gsr_mag[i] for i in range(6) if i != global_pair_index]
        assert all(v == 0 for v in other)

    def test_set_gsr_overwrites_previous_values(self):
        state = SensorState()
        state.set_gsr(3, mag=0.1, phase=0.2)
        state.set_gsr(3, mag=0.8, phase=3.0)
        _, gsr_mag, gsr_phase = state.snapshot()
        assert gsr_mag[3] == pytest.approx(0.8)
        assert gsr_phase[3] == pytest.approx(3.0)


class TestSnapshot:
    def test_snapshot_returns_three_values(self):
        state = SensorState()
        result = state.snapshot()
        assert len(result) == 3

    def test_snapshot_cap_is_copy(self):
        state = SensorState()
        state.set_cap(0, 0.5)
        cap, _, _ = state.snapshot()
        cap[0] = 999
        cap2, _, _ = state.snapshot()
        assert cap2[0] == pytest.approx(0.5)

    def test_snapshot_gsr_mag_is_copy(self):
        state = SensorState()
        state.set_gsr(0, mag=0.4, phase=0.0)
        _, gsr_mag, _ = state.snapshot()
        gsr_mag[0] = 999
        _, gsr_mag2, _ = state.snapshot()
        assert gsr_mag2[0] == pytest.approx(0.4)

    def test_snapshot_gsr_phase_is_copy(self):
        state = SensorState()
        state.set_gsr(0, mag=0.0, phase=1.5)
        _, _, gsr_phase = state.snapshot()
        gsr_phase[0] = 999
        _, _, gsr_phase2 = state.snapshot()
        assert gsr_phase2[0] == pytest.approx(1.5)


class TestThreadSafety:
    def test_concurrent_writes_and_reads_do_not_crash(self):
        state = SensorState()
        errors = []

        def write_caps():
            try:
                for _ in range(200):
                    for i in range(4):
                        state.set_cap(i, float(i) * 0.1)
            except Exception as e:
                errors.append(e)

        def write_gsr():
            try:
                for _ in range(200):
                    for i in range(6):
                        state.set_gsr(i, mag=float(i) * 0.05, phase=float(i) * 0.2)
            except Exception as e:
                errors.append(e)

        def read_state():
            try:
                for _ in range(200):
                    state.snapshot()
            except Exception as e:
                errors.append(e)

        threads = (
            [threading.Thread(target=write_caps) for _ in range(3)]
            + [threading.Thread(target=write_gsr) for _ in range(3)]
            + [threading.Thread(target=read_state) for _ in range(3)]
        )
        for t in threads:
            t.start()
        for t in threads:
            t.join()

        assert errors == []


class TestConstants:
    def test_gsr_pairs_value(self):
        assert GSR_PAIRS == [(1, 2), (1, 3), (1, 4), (2, 3), (2, 4), (3, 4)]

    def test_gsr_pairs_length(self):
        assert len(GSR_PAIRS) == 6

    def test_node_gsr_mapping_value(self):
        # Node 0: no GSR RX
        # Node 1: global 0
        # Node 2: globals 1, 3
        # Node 3: globals 2, 4, 5
        assert NODE_GSR_MAPPING == [[], [0], [1, 3], [2, 4, 5]]

    def test_node_gsr_mapping_length(self):
        assert len(NODE_GSR_MAPPING) == 4

    def test_node_gsr_mapping_node0_empty(self):
        assert NODE_GSR_MAPPING[0] == []

    def test_node_gsr_mapping_node1(self):
        assert NODE_GSR_MAPPING[1] == [0]

    def test_node_gsr_mapping_node2(self):
        assert NODE_GSR_MAPPING[2] == [1, 3]

    def test_node_gsr_mapping_node3(self):
        assert NODE_GSR_MAPPING[3] == [2, 4, 5]
